...
form.addEventListener("submit", function (e) {
  // Pour ce cas la car nous avons un button sumbit qui soumet donc un formulaire s'il est vide par default il raffrachira la page, preventDefault empeche le comportement par default il empeche donc le raffrachissement de la page
  e.preventDefault();
  const random = Math.floor(Math.random() * text.length);
  // parseInt = convertit le premier argument en une chaîne, l'analyse et renvoie un entier
  const value = parseInt(amount.value);

  if (isNaN(value) || value < 1 || value > 9) {
    result.innerHTML = `<p class = "result" align = "center">${text[random]} </p><br>`;
  } else {
    // slice renvoie un objet tableau, contenant une copie superficielle d'une portion du tableau d'origine le premier argument sert a indiquer par quel chiffre on commence, si on ne met rien il fera l'inverse de 0
